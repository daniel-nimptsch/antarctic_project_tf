...
#### Working directory ####
setwd(paste(own_cloud_dir, "/Arbeit_SAG/Pipeline_Results/Antarctis_1_NGS/Antarctis_1_NGS_2020/final_tables_for_grafics/heatmaps", sep = ""))
...
p = plot_heatmap(physeq, low="#000033", high="#CCFF66", taxa.label = "species.taxonomy", taxa.order = taxa_order,
                 title = title, sample.order = sample_oder) + theme(plot.title = element_text(size=11))
p + theme(axis.text.x = element_text(angle = -45), axis.text.y = element_text(size = 9, angle = 0))
dev.off()
