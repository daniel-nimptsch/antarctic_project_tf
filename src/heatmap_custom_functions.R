...
#
# Custom edit from the plot_heatmap function from the phyloseq package
#
                              ...
                              ...) 
{
  ...
  if (!is.null(method) & (is.null(taxa.order) | is.null(sample.order))) {
    junk = capture.output(ps.ord <- ordinate(physeq, method, 
                                             distance, ...), file = NULL)
    if (is.null(sample.order)) {
      siteDF = NULL
      trash1 = try({
        siteDF <- scores(ps.ord, choices = c(1, 2), display = "sites", 
                         physeq = physeq)
      }, silent = TRUE)
  ...
  if (!is.null(trans)) {
    p = p + scale_fill_gradient(low = low, high = high, trans = trans, 
                                na.value = na.value)
  }
  else {
